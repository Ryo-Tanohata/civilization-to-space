...
namespace CivilizationToSpace.View
{
    ...
    public sealed class EarthView : MonoBehaviour
    {
        ...
        ///
        /// **なぜ45秒から10秒へ速めたのか。**
        /// 季節を見せるには、1年が1日の数倍なければならないためである
        /// （<see cref="AppRoot.YearSeconds"/> に理由がある）。1周45秒のままだと、
        /// 通しで見ても季節が半周も進まない。ここでは1年に4回まわる。
        ///
        /// **なぜ1秒ではないのか。**
        /// はじめ1周1秒にしたところ、速すぎて地表の模様を目で追えなかった。
        /// 1周10秒なら、大陸が流れていくのを追いながら季節の移りも見られる。
        /// </summary>
        private const float SpinDegreesPerSecond = 36f;

        /// <summary>
        /// 雲を地表より少し速く流す（度／秒）。
        ///
        /// 差の12度／秒だけが目に見える。雲は30秒ほどかけて地表を1周ぶん追い越す。
        /// 自転を変えるときも、比ではなくこの差を保つ。比で決めると、
        /// 自転が速いときに雲が地表の上を滑らず、別々に回る2枚の球に見える。
        /// </summary>
        private const float CloudDegreesPerSecond = 48f;

        /// <summary>
        /// 衛星の周回の速さ（度／秒）。1周およそ26秒。
        ///
        /// **自転を変えたが、ここは変えていない。** 変えると小さな点が輪になって
        /// 尾を引き、機体の数が読めなくなるためである。1周26秒は1日10秒に対して
        /// 2.6日にあたり、実際の低軌道衛星（1日およそ16周）とは桁が違う。
        /// 高度・速度・周期のいずれも表していない。
        /// </summary>
